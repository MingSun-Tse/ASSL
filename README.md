...
Neural network pruning is typically conducted on a *pretrained* model. Our method also follows this common practice. Before we run the pruning scripts next, here we set up the pretrained dense models, which can be downloaded from here (TODO).
...
### Visual Results
Visual comparison (x4) among lightweight SR approaches on the Urban100 dataset is shown below. For the complete visual results generated by our method, please refer to here (TODO).
